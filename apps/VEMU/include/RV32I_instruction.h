...
typedef uint32_t (*rv32i_inst_exec_fn_t)(rv32i_decoded_inst_t decoded_inst);
typedef uint32_t (*rv32i_inst_imm_fn_t)(rv32i_inst_t inst, uint32_t *imm);
...
typedef struct {
    ...
    uint8_t funct3;
    uint8_t funct7;
    rv32i_inst_imm_fn_t rv32i_inst_imm_fn;
    rv32i_inst_exec_fn_t rv32i_inst_exec_fn;
} rv32i_base_inst_t;

#define RV32I_INST_DECLARE(table, name, ty, op, f3, f7, imm_func_ptr, exec_func_ptr) do { \
    int idx = (table).count; \
    if (idx < RV32I_MAX_INSTRUCTIONS) { \
        ...
        (table).instructions[idx].rv32i_inst_imm_fn = imm_func_ptr; \
        (table).instructions[idx].rv32i_inst_exec_fn = exec_func_ptr; \
        (table).count = idx + 1; \
    } \
} while(0)
